      PROGRAM INTERACTIVECM4DRIVER
*
********************************************************************************
*
* Function:
*
* Interactive B and J evaluator for the CM4 model series.
*
* Steps:
*
* 1) Set the PATH variable in the DATA statement below:
*
*    PATH(1) = absolute path of model file to be used,
*    PATH(2) = absolute path of Dst table file to be used, if needed,
*    PATH(3) = absolute path of F10.7 table file to be used, if needed.
*
*    (see the cm4field.f documentation for more info)
*
* 2) Compile on Linux as:
*
*    g77 -O -o interactive_cm4 interactive_cm4.f
*
* 3) Invoke as:
*
*    ./interactive_cm4
...
*
      ...
      LOGICAL LOAD(3),INDX(2),GMUT,CORD,PRED(6),CURR,COEF
      INTEGER PERR,OERR,CERR
      INTEGER UNIT(3),NHMF(2),NLMF(2)
      DOUBLE PRECISION MUT
      DOUBLE PRECISION JMDL(3,4)
      DIMENSION BMDL(3,7),GMDL(1)
********************************************************************************
      DATA PATH /
     1       '/home/sabaka/CM4/umdl.CM4',
     2       '/home/sabaka/CM4/Dst_all.wdc',
     3       '/home/sabaka/CM4/F107_3mon.plt'
     4/
********************************************************************************
      DATA UNIT /10, 20, 30/
      DATA LOAD /.TRUE., .TRUE., .TRUE./
      DATA COEF /.FALSE./
      DATA PERR /1/
      DATA OERR /6/
      DATA CERR /0/
   ...
   60    WRITE(6,1009)
         ...
         IF (RESPONSE.EQ.'y') THEN
            PRED(1)=.TRUE.
            WRITE(6,1010)
            READ(5,*) NLMF(1),NHMF(1)
            WRITE(6,1011)
            READ(5,*) NLMF(2),NHMF(2)
         ELSE IF (RESPONSE.EQ.'n') THEN
            PRED(1)=.FALSE.
         ELSE
            WRITE(6,1001)
  ...
  110          WRITE(6,1016)
         ...
         READ(5,*) ALT
         WRITE(6,1021)
         CALL CM4FIELD (PATH,UNIT,LOAD,INDX,GMUT,CORD,PRED,CURR,
     1                  COEF,NHMF,NLMF,UT,MUT,THETA,PHI,ALT,DST,
     2                  F107,BMDL,JMDL,GMDL,PERR,OERR,CERR)
         IF (CERR.GT.49) GO TO 120
         IF (PRED(1)) THEN
            WRITE(6,1022) ((BMDL(J,K),J=1,3),K=1,2)
         END IF
         IF (CURR) THEN
            IF (PRED(2)) THEN
               WRITE(6,1023) ((BMDL(J,K),J=1,3),K=3,4),
     1                       (JMDL(J,1),J=1,3)
            END IF
            IF (PRED(3)) THEN
               WRITE(6,1024) ((BMDL(J,K),J=1,3),K=5,6),
     1                       ((JMDL(J,K),J=1,3),K=2,3)
            END IF
            IF (PRED(4)) THEN
               WRITE(6,1025) (BMDL(J,7),J=1,3),
     1                       (JMDL(J,4),J=1,3)
            END IF
         ELSE
            IF (PRED(2)) THEN
               WRITE(6,1026) ((BMDL(J,K),J=1,3),K=3,4)
            END IF
            IF (PRED(3)) THEN
               WRITE(6,1027) ((BMDL(J,K),J=1,3),K=5,6)
            END IF
            IF (PRED(4)) THEN
               WRITE(6,1028) (BMDL(J,7),J=1,3)
            END IF
         END IF
         WRITE(6,1029)
         READ(5,*) RESPONSE
      IF (RESPONSE.EQ.'y') GO TO 10
  120 STOP
 1000 FORMAT(' ',/,1X,'----------- Interactive B and J Field Evaluator f
     1or CM4 ------------',/,' ',/,1X,'Compute J along with B? (y/n)')
 1001 FORMAT(4X,'...response not recognized...')
 1002 FORMAT(1X,'Get Dst from file? (y/n)')
 1003 FORMAT(4X,'...enter Dst value:')
 1004 FORMAT(1X,'Get F10.7 from file? (y/n)')
 1005 FORMAT(4X,'...enter F10.7 value:')
 1006 FORMAT(1X,'Compute MUT from UT? (y/n)')
 1007 FORMAT(4X,'...enter MUT value (0-24 hrs):')
 1008 FORMAT(1X,'Geodetic (d) or geocentric (c) coordinates?')
 1009 FORMAT(1X,'Use core/lithospheric sources? (y/n)')
 1010 FORMAT(4X,'...enter min and max degree range for first evaluation:
     1')
 1011 FORMAT(4X,'...enter min and max degree range for second evaluation
     1:')
 1012 FORMAT(1X,'Use magnetospheric sources? (y/n)')
 1013 FORMAT(1X,'Use ionospheric sources? (y/n)')
 1014 FORMAT(1X,'Use insitu (coupling) current sources? (y/n)')
 1015 FORMAT(4X,'...from Magsat instead of Oersted? (y/n)')
 1016 FORMAT(7X,'...from dawn LT instead of dusk LT? (y/n)')
 1017 FORMAT(1X,'Enter UT value (yr A.D.):')
 1018 FORMAT(1X,'Enter THETA value (deg):')
 1019 FORMAT(1X,'Enter PHI value (deg):')
 1020 FORMAT(1X,'Enter ALT value (km):')
 1021 FORMAT(' ',/,1X,'...computing...',/,' ')
 1022 format(1x,'C/L 1   B_xyz    ',3e15.8,/,
     1       1x,'C/L 2   B_xyz    ',3e15.8)
 1023 format(1x,'Mag_pri B_xyz    ',3e15.8,/,
     1       1x,'Mag_ind B_xyz    ',3e15.8,/,
     2       1x,'Mag_ind J_xy,Psi ',3e15.8)
 1024 format(1x,'Ion_pri B_xyz    ',3e15.8,/,
     1       1x,'Ion_ind B_xyz    ',3e15.8,/,
     2       1x,'Ion_pri J_xy,Psi ',3e15.8,/,
     3       1x,'Ion_ind J_xy,Psi ',3e15.8)
 1025 format(1x,'Tor     B_xyz    ',3e15.8,/,
     1       1x,'Pol     J_xyz    ',3e15.8)
 1026 format(1x,'Mag_pri B_xyz    ',3e15.8,/,
     1       1x,'Mag_ind B_xyz    ',3e15.8)
 1027 format(1x,'Ion_pri B_xyz    ',3e15.8,/,
     1       1x,'Ion_ind B_xyz    ',3e15.8)
 1028 format(1x,'Tor     B_xyz    ',3e15.8)
 1029 FORMAT(' ',/,1X,'Would you like to continue? (y/n)',/,' ')
      END
