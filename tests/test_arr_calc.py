...
def Core_unit_test(filepath = 'test_values/Core_unittest_inputs.csv'):
    ...
    testval_dir = os.path.join(test_dir, "test_values")
    ...
    for i in range(0,len(data)):
        ...
        if(not np.all(np.isclose(answers[i],core,rtol = 1e-4))):
            raise AssertionError('Do matlab and py cm4_core dont agree to 5 digits',np.isclose(answers[i],core,rtol = 1e-4), answers[i], core)
        else:
...
def Magnetosphere_Unit_test(filepath = 'test_values/Core_unittest_inputs.csv'):
    ...
    for i in range(0,len(data)):
        ...
        if(not np.all(np.isclose(answers[i],magnetosphere,rtol = 1e-4))):
            raise AssertionError ('Do matlab and py cm4_magnetosphere dont agree to 5 digits',np.isclose(answers[i],magnetosphere,rtol = 1e-4), answers[i], magnetosphere)
        else:
...
def Ionosphere_Unit_test(filepath = 'test_values/Core_unittest_inputs.csv'):
    ...
    for i in range(0,len(data)):
        ...
        if(not np.all(np.isclose(answers[i],ionosphere,rtol = 1e-4))):
            raise AssertionError('Do matlab and py cm4_Ionosphere dont agree to 5 digits',np.isclose(answers[i],ionosphere,rtol = 1e-4), answers[i], ionosphere)
        else:
...
def run_unit_tests():
    ...
    test_dir = os.path.dirname(__file__)
    testval_dir = os.path.join(test_dir, "test_values")
    core_testval_file = os.path.join(testval_dir, "Core_unittest_inputs.csv")

    Ionosphere_Unit_test( core_testval_file)
    Magnetosphere_Unit_test( core_testval_file)
    Core_unit_test( core_testval_file)
def read_testval_inputs():
    test_dir = os.path.dirname(__file__)
    testval_dir = os.path.join(test_dir, "test_values")
    core_testval = os.path.join(testval_dir, "Core_unittest_inputs.csv")
    filepath = core_testval
    data = []
...
def arr_calc_unittests():
    ...
    core_testval = os.path.join(testval_dir, "testvalBcore.csv")
    answers = read_answers(core_testval)
    preds = np.array([True, False, False, False, False, False])
    dst = [7.]*len(lat)
    f107 = [723.]*len(lat)
    out_b,out_j, core,crust, magnetosphere, ionoshere = py_mat_cm4_arr(np.zeros(len(lat)), np.array(lat,dtype= 'f'), np.array(lon, dtype='f'), dst, f107, pred = preds, year= year, month= month, day= day, hour= hour, minute= minute)
    core = np.transpose(core)
    all_pass = True
    #print(core)
    for i in range(0, len(lat)):
        if(not np.all(np.isclose(answers[i],core[i],rtol = 1e-4))):
            raise AssertionError('Do matlab and py cm4_core dont agree to 5 digits',np.isclose(answers[i],core,rtol = 1e-4), answers[i], core[i])
            all_pass = False
    ...
    for i in range(0, len(lat)):
        if(not np.all(np.isclose(answers[i],magnetosphere[i],rtol = 1e-4))):
            print('Do matlab and py cm4_magneto dont agree to 5 digits',np.isclose(answers[i],magnetosphere[i],rtol = 1e-4), answers[i], magnetosphere[i])
            all_pass = False
